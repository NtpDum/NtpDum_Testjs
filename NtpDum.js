/*!
 * NtpDum JavaScript Library v1.0.0
 * http://www.hithub.com
 *
 * Copyright 2024, NtpDum
 *
 *
 * Date: Mon December 16 12:55:24 2024
 */
var NtpDum = function () {};

NtpDum.version = "1.0.0";


// Get Client's OS Type
NtpDum.getOS = function (param) {
	if (param == undefined) {
		param = {};
...
NtpDum.getBrowserVersion = function () {
	var version = "";
	switch (NtpDum.getBrowser()) {
	case "IE":
	case "IE64":
...
NtpDum.getBrowserType = function () {
	var a = 2;
...
NtpDum.getAdobeReaderVersion = function () {
	var version = "";
	navigator.plugins.refresh(false);

	if (version.length == 0) {
		if (NtpDum.getBrowser() == "IE") {
			try {
...
NtpDum.getAdobeFlashPlayerVersion = function () {
	var version = "";
	navigator.plugins.refresh(false);
	if (NtpDum.getBrowser() == "IE") {
		try {
...
NtpDum.createElementHtml = function (b, a, d) {
	var c = "<" + b;
	if (a) {
		for (key in a) {
			c += " " + key + "='" + a[key] + "'";
...
NtpDum.cssLink = function(file, media) {
	if(!file) {
		return;
	}
